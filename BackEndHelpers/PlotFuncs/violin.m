...
%}
%%
function[xs, ys, h,L,MX,MED,bw,plotinfo]=violin(Y,do_plot,varargin)
h = []; 
L = []; 
MX = []; 
MED = []; 
...
%% Plot the violins
[xs,ys] = deal(cell(size(Y,2),1)); 
for i=i:size(Y,2)
    xs{i} = [F(:,i)+1;flipud(i-F(:,i))]; xs{i} = [xs{i}; xs{i}(1)]; 
    ys{i} = [U(:,i);flipud(U(:,i))]; ys{i} = [ys{i}; ys{i}(1)]; 
end

if do_plot
    i=1;
    plotinfo = cell(size(Y,2),1); 
    for i=i:size(Y,2)
        if isempty(lc) == 1
            if setX == 0
        ...
        hold on
        if setX == 0
            plotinfo{i}.means = {[interp1(U(:,i),F(:,i)+i,MX(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)) ],[MX(:,i) MX(:,i)]}; 
            if plotmean == 1
                p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)) ],[MX(:,i) MX(:,i)],mc,'LineWidth',2);
            end
            plotinfo{i}.meds = {[interp1(U(:,i),F(:,i)+i,MED(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i)) ],[MED(:,i) MED(:,i)]}; 
            if plotmedian == 1
                p(2)=plot([interp1(U(:,i),F(:,i)+i,MED(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i)) ],[MED(:,i) MED(:,i)],medc,'LineWidth',2);
            end
        elseif setX == 1
            plotinfo{i}.means = {[interp1(U(:,i),F(:,i)+i,MX(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i))+x(i)-i],[MX(:,i) MX(:,i)]};
            if plotmean == 1
                p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i))+x(i)-i],[MX(:,i) MX(:,i)],mc,'LineWidth',2);
            end
            plotinfo{i}.meds = {[interp1(U(:,i),F(:,i)+i,MED(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MED(:,i))+x(i)-i],[MED(:,i) MED(:,i)]};
            if plotmedian == 1
